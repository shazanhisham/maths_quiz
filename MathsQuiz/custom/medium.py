def medium():
        ...
        for i in range(1,(qnum+1)):
                ...
                operand1=(random.randint(0,50))
                L2.append(operand1)#append Operand1 to list L2
        
                operand2=(random.randint(0,50))
                L2.append(operand2)#append Operand2 to list L2

                operator=random.choice(op)
                L2.append(operator)#append Operator to list L2
                
                print((str(i))+") ",operand1,operator,operand2,"=",end="")   
        ...
        print("You played Custom Game with Medium mode",end="")
        print("\nYou answered "+str(len(L1))+" questions\n")

        #Checking user input answer with correct answer
        for i in range(len(L1)):
                if L1[i][2]=="+":
                        print(str(i+1)+")",(L1[i][0]),(L1[i][2]),(L1[i][1]),"=",(L1[i][3]),end="")
                        ans = (L1[i][0])+(L1[i][1])
                        if (ans)==int(L1[i][3]):
                        ...
                        print(str(i+1)+")",(L1[i][0]),(L1[i][2]),(L1[i][3]),"=",(L1[i][3]),end="")
                        ans = (L1[i][0])-(L1[i][1])
                        if (ans)==int(L1[i][3]):
                                print(" (answer" + str(ans) +") [Correct]")
                                count+=1
